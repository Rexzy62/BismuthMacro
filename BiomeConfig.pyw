...
class MultiSelectDropdown(tk.Frame):
    ...
    def _show_multi_select_dialog(self, _event=None):
...
def load_settings():
    # The config file might be saved with UTF-16 encoding, which needs to be specified.
    if os.path.exists(CONFIG_FILE):
        try:
            config.read(CONFIG_FILE, encoding='utf-16')
        except UnicodeError:
            config.read(CONFIG_FILE, encoding='utf-8') # Fallback to utf-8
    
    settings = {}
    if 'Main2' in config:
        settings['PrivateServerLinkEnable'] = config.getboolean('Main2', 'PrivateServerLinkEnable', fallback=False)
        
        send_message_str = config.get('Main2', 'EnableOnThisBiome', fallback='')
        settings['EnableOnThisBiome'] = [item.strip() for item in send_message_str.split(',') if item.strip()]

        ping_on_str = config.get('Main2', 'PingOnThisBiome', fallback='')
        settings['PingOnThisBiome'] = [item.strip() for item in ping_on_str.split(',') if item.strip()]

        settings['StatusEnabled'] = config.getboolean('Main2', 'StatusEnabled', fallback=False)
        settings['EmojiEnabled'] = config.getboolean('Main2', 'EmojiEnabled', fallback=False)
    return settings

def save_settings():
    if 'Main2' not in config:
        config['Main2'] = {}
    
    # Save all settings from the GUI
    config['Main2']['PrivateServerLinkEnable'] = str(private_server_link_var.get())
    config['Main2']['EnableOnThisBiome'] = ", ".join(send_message_dropdown.get_selected())
    config['Main2']['PingOnThisBiome'] = ", ".join(ping_dropdown.get_selected())
    config['Main2']['StatusEnabled'] = str(enable_aura_var.get())
    config['Main2']['EmojiEnabled'] = str(enable_emojis_var.get())

    # The config file might be read with utf-16, so we save it with the same encoding to be safe.
    try:
        with open(CONFIG_FILE, 'w', encoding='utf-16') as configfile:
            config.write(configfile)
    except Exception:
        # Fallback to utf-8 if utf-16 fails for some reason
