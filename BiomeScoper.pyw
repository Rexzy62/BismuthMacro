...
def read_config_from_ini(ini_path):
    ...
    try:
        ...
        if 'Main2' in config:
            print(f"DEBUG: Section '[Main2]' found.")
            main2_config = config['Main2']

            settings['WebhookLink'] = main2_config.get('WebhookLink')
            if settings['WebhookLink']:
                print(f"DEBUG: Successfully read WebhookLink (first 10 chars): {settings['WebhookLink'][:10]}...")
            else:
                print(f"ERROR: Key 'WebhookLink' not found in section '[Main2]'. Webhook will not be sent.")

            settings['PSLink'] = main2_config.get('PSLink')
            if settings['PSLink']:
                print(f"DEBUG: Successfully read PSLink (first 10 chars): {settings['PSLink'][:10]}...")
            else:
                print(f"ERROR: Key 'PSLink' not found in section '[Main2]'. PSLink will not be added to embed.")
            
            settings['PrivateServerLinkEnable'] = main2_config.getboolean('PrivateServerLinkEnable', False)
            print(f"DEBUG: PrivateServerLinkEnable: {settings['PrivateServerLinkEnable']}")
            
            enable_biomes_str = main2_config.get('EnableOnThisBiome', '')
            ...
            ping_biomes_str = main2_config.get('PingOnThisBiome', '')
            ...
            settings['StatusEnabled'] = main2_config.getboolean('StatusEnabled', False)
            print(f"DEBUG: StatusEnabled: {settings['StatusEnabled']}")

            settings['EmojiEnabled'] = main2_config.getboolean('EmojiEnabled', False)
            print(f"DEBUG: EmojiEnabled: {settings['EmojiEnabled']}")

            settings['DiscordUserId'] = main2_config.get('DiscordUserId')
            if settings['DiscordUserId']:
                print(f"DEBUG: DiscordUserId: {settings['DiscordUserId']}")

        else:
            print(f"ERROR: Section '[Main2]' not found in {ini_path}. Default settings will be used.")
